...
def update_layout(
    ...
    fig.update_layout(
        ...
        xaxis_title=dict(text=x_label, font=dict(weight="bold")),
        yaxis_title=dict(text=y_label, font=dict(weight="bold")),
        template=template,
        legend=dict(
            title=dict(text="Time Series", font=dict(weight="bold", size=16)),
            x=1.01,
            y=0.935,
        ),
    )
...
def plot_time_series_dataframe(
    dataframes: Collection[pd.DataFrame],
    titles: Sequence[str],
    x_label: str = "Event Date",
    y_label: str = "Water Level (m)",
    template: str = "plotly",
    ...
    LOGGER.debug(i18n.t("tide.plot.generating_plots", titles=titles))
